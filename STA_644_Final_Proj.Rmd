...
```{r}
#EDA
lm_data = sp_data %>% mutate(lg_viol15 = log(viol15), lg_hhpov15 = log(hhpov15),
                          lg_comprop15 = log(comprop15), lg_numbus15 = log(numbus15),
                          lg_tanf15 = log(tanf15), lg_vacant15 = log(vacant15)) %>%
  select(-c(viol15,hhpov15,comprop15,numbus15,tanf15,vacant15))
GGally::ggpairs(lm_data[,c(11,2:6)])
GGally::ggpairs(lm_data[,c(11,7:10)])
GGally::ggpairs(lm_data[,c(11:16)])
```

```{r}
#variable selection using BMA
library(BAS)
df.bas = bas.lm(lg_viol15 ~ ., data=lm_data[,-1],
       prior="g-prior", a=nrow(lm_data), modelprior=uniform())
...
par(mfrow=c(1,1))
bas_pred = lm_data[,c("CSA2010",names(lm_data %>% select(-lg_viol15))[df.bas$probne0[-1]>=0.5])]
bas_data = left_join(sp_resp,bas_pred,by="CSA2010")

#variable selection using stepwise selection
reg1 = lm(lg_viol15~.,data = lm_data)
...
ggplot()+
  ...
  geom_sf(data=bas_data, aes(fill=viol15))+
